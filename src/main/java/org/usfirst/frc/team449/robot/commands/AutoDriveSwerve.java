 ...
 * DriveSwerve requested distance using bang bang control
...
public class AutoDriveSwerve extends Command {
	...
	private boolean isForward; //is the distance positive or negative

	private Timer t;// timer for time checking
	public AutoDriveSwerve(double distance, double time) {
		requires(Robot.drive);
		this.initDist = distance;
		this.initTim = time;
		Robot.drive.setManual(true);
	}
